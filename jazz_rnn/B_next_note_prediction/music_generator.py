...
class MusicGenerator:
    ...
    def __init__(self,
        ...
        if score_model == 'harmony':
            self.seq_len = 2
            self.score_inference = HarmonyScoreInference(
                converter, beam_width, beam_depth, batch_size)
            self.score_model = 'harmony'
        elif score_model != '':
            self.score_inference = ScoreInference(score_model,
                                                  converter,
                                                  beam_width,
                                                  threshold,
                                                  batch_size,
        ...
        self.early_start = (not no_head) and (self.song
                                              in early_start_song_dict)
    ...
    def get_non_x_dur_idxs_(self, x):
        ...
        if len(dur_list) == len(self.converter.bidict.items()):
            return torch.tensor(self.converter.dur_2_ind(4),
                                device=self.device)
        return torch.tensor(dur_list, device=self.device)
    ...
    def init_stream(self, xml_file):
        ...
        self.head_len = len(
            self.chords
        )  # head_length can be defined in args for sequential generation

    def insert_starting_notes(self, xml_file):
        """insert notes to generator for the generator to continue"""
        data = extract_vectors(song=xml_file,
                               ri=False,
                               song_labels_dict={},
                               converter=self.converter)
        data = np.array(data)
        data = torch.as_tensor(data, device=self.device)
        data = data.unsqueeze(1).expand(data.shape[0], self.batch_size,
                                        data.shape[-1])
        ...
        with torch.no_grad():
            if isinstance(self.model, ChordPitchesModel):
                _, _, self.initial_hidden = self.model(data,
                                                       self.initial_hidden)
            else:
                _, _, self.initial_hidden, self.attentions = self.model._forward(
                    data, [], self.initial_hidden)
        first_chord = self.chords[0][0]
        root, scale_pitches, chord_pitches, chord_idx = chord_2_vec(
            first_chord)
        self.initial_last_note_net = data.new(
            (list(data[-1, 0, :3]) + [root] + scale_pitches + chord_pitches +
             [chord_idx])).expand(1, self.batch_size, data.shape[-1])
    ...
    def generate_measures(self, n_measures):
        ...
        notes = []
        residuals_m21 = np.ndarray([self.batch_size, RES_LENGTH],
                                   dtype=Fraction)
        ...
        with torch.no_grad():
            ...
            for measure_idx in tqdm(measure_idxs):
                ...
                last_note_net, total_offset, hidden, residuals_m21, notes, top_score = self.generate_one_measure(
                    measure_idx, n_measures, notes, residuals_m21,
                    last_note_net, total_offset, hidden)
                    ...
                    if measure_idx % self.beam_depth == 0:
                        if self.score_inference is not None:
                            topk_batch_indices, top_score = self.score_inference.get_topk_batch_indices_from_notes(
                                self.notes_net)
                            self.notes_net = self.general_repeat(
                                self.notes_net, topk_batch_indices, dim=1)
                        else:
                            topk_batch_indices, top_score = get_topk_batch_indices_from_notes(
                                n, self.beam_width)

                        # select k notes and repeat batch_size/beam_width times:
                        notes = list(
                            self.general_repeat(n, topk_batch_indices, dim=1))
                        residuals_m21 = self.general_repeat(residuals_m21,
                                                            topk_batch_indices,
                                                            dim=0)
                        last_note_net = self.general_repeat(
                            last_note_net, topk_batch_indices, 1)
                        total_offset = self.general_repeat(
                            total_offset, topk_batch_indices, 0)
                        hidden = self.general_repeat(hidden,
                                                     topk_batch_indices, 1)
                else:
                    if self.beam_search != 'note':
                        number_of_notes_in_measure = np.count_nonzero(
                            n[:, :,
                              LOG_PROB_IDX_IN_NOTE], axis=0)[np.newaxis, :]
                        number_of_notes_in_measure = number_of_notes_in_measure.repeat(
                            n.shape[0], 0)
                        measure_log_likelihood = (
                            n[:, :, LOG_PROB_IDX_IN_NOTE] /
                            number_of_notes_in_measure).sum(axis=0)
    ...
    def general_repeat(self, x, topk_batch_indices, dim):
        if isinstance(x, np.ndarray):
            ...
            return np.tile(x.take(topk_batch_indices, axis=dim),
                           tuple(shape.tolist()))
            ...
            return tuple(
                (self.general_repeat(t, topk_batch_indices, dim) for t in x))
        elif isinstance(x, list):
            return list(
                (self.general_repeat(t, topk_batch_indices, dim) for t in x))

    def generate_one_measure(self, measure_idx, n_measures, notes,
                             residuals_m21, last_note_net, total_offset,
                             hidden):
        all_workers_done = False
        measure_done = torch.zeros([self.batch_size],
                                   dtype=torch.uint8,
                                   device=self.device)
        measure_not_done = 1 - measure_done
        duration_in_measure_for_debug = np.zeros([self.batch_size],
                                                 dtype=Fraction)
        ...
        while not all_workers_done:
            ...
            last_dur_nonzero_mask = torch.as_tensor(
                last_dur_m21.astype(np.float32),
                device=self.notes_net.device) != 0
            not_residual_mask = torch.as_tensor(
                prev_residuals[:, 1].astype(np.float32),
                device=self.notes_net.device) == 0
            update_mask = (last_dur_nonzero_mask.unsqueeze(-1)) & (
                not_residual_mask.unsqueeze(-1))

            self.notes_net = torch.where(
                update_mask, torch.cat((self.notes_net[1:], last_note_net)),
                self.notes_net)
            ...
            if self.beam_search == 'note' and len(
                    notes) % self.beam_depth == 0:
                n = np.array(notes)
                if self.score_inference is not None:
                    topk_batch_indices, top_score = self.score_inference.get_topk_batch_indices_from_notes(
                        self.notes_net)
                    self.notes_net = self.general_repeat(self.notes_net,
                                                         topk_batch_indices,
                                                         dim=1)
                else:
                    topk_batch_indices, top_score = get_topk_batch_indices_from_notes(
                ...
                residuals_m21 = self.general_repeat(residuals_m21,
                                                    topk_batch_indices,
                                                    dim=0)
                last_note_net = self.general_repeat(last_note_net,
                                                    topk_batch_indices, 1)
                total_offset = self.general_repeat(total_offset,
                                                   topk_batch_indices, 0)
                hidden = self.general_repeat(hidden, topk_batch_indices, 1)
                duration_in_measure_for_debug = self.general_repeat(
                    duration_in_measure_for_debug, topk_batch_indices, 0)
                measure_done = self.general_repeat(measure_done,
                                                   topk_batch_indices, 0)
                measure_not_done = 1 - measure_done
            else:
                top_score = 0
    ...
    def generate_one_note(self, duration_in_measure_for_debug, hidden,
                          last_note_net, measure_done, measure_idx,
                          measure_not_done, n_measures, notes, residuals_m21,
                          total_offset):
        # predict next note
        enforce_pitch = None
        enforce_dur = None
        ...
        residual_exists_mask = self.handle_residual(measure_done, new_dur_m21,
                                                    new_pitch, new_tie,
                                                    residuals_m21)
        ...
        new_offset = torch.as_tensor((self.fraction_2_float(bar_offset) * 12),
                                     dtype=torch.long,
                                     device=self.device)
        ...
        at_second_half_of_bar = ((bar_offset >= 2) + cross_end_bar_mask +
                                 end_at_end_bar) > 0

        # handle notes that crossed end of bar
        if cross_end_bar.size != 0:
            previous_bar_offset = (
                (bar_offset[cross_end_bar] - new_dur_m21[cross_end_bar]) % 4)
            duration_until_end_bar = 4 - previous_bar_offset
            residual_duration_m21 = new_dur_m21[
                cross_end_bar] - duration_until_end_bar
            new_dur_m21[cross_end_bar] = duration_until_end_bar
            new_tie[cross_end_bar] = tie_2_value['start']
            residuals_m21[cross_end_bar] = np.concatenate(
                (new_pitch[cross_end_bar].cpu().numpy(),
                 residual_duration_m21[:, np.newaxis],
                 new_note_log_prob[cross_end_bar][:, np.newaxis]),
                axis=1)
            total_offset[cross_end_bar] = total_offset[
                cross_end_bar] - residual_duration_m21
        ...
        last_note_in_measure_mask = torch.as_tensor(
            (end_at_end_bar | cross_end_bar_mask).astype(np.long),
            dtype=torch.long,
            device=self.device)
        ...
        next_chord = [
            self.chords[measure_idx % self.head_len][int(c)]
            if not bool(last_note_in_measure_mask[ind]) else
            self.chords[(measure_idx + 1) % self.head_len][0]
            for ind, c in enumerate(at_second_half_of_bar)
        ]
        root, scale_pitches, chord_pitches, chord_idx = \
            chord_2_vec_on_tensor(next_chord, device=self.device)
        new_notes_for_net = torch.cat(
            (new_pitch, new_dur_net, new_offset.unsqueeze(1),
             root.unsqueeze(1), scale_pitches, chord_pitches,
             chord_idx.unsqueeze(1)),
            dim=1)
        # update last note only where measure is not done and finished handling residuals
        update_last_note_batch_mask = (measure_not_done *
                                       (1 - residual_exists_mask.byte()))
        update_last_note_mask = update_last_note_batch_mask.unsqueeze(
            -1).expand(self.batch_size, NOTE_VECTOR_SIZE).byte()
        ...
        measure_idx_tensor = torch.as_tensor(measure_idx,
                                             ...
                                             device=self.device).expand(
                                                 self.batch_size)
        new_notes_m21 = np.stack(
            (new_pitch.squeeze().cpu().detach().numpy(), new_dur_m21,
             new_tie.cpu().detach().numpy(), measure_idx_tensor.cpu().numpy(),
             new_note_log_prob),
            axis=1)
        notes.append(new_notes_m21)
        update_hiddens_mask = (measure_not_done.long() +
                               residual_exists_mask) > 0
        for i in range(len(hidden)):
            if hidden[0].shape[0] == new_hidden[0].shape[0]:
                hidden[i][:, update_hiddens_mask.nonzero(
                ), :] = new_hidden[i][:, update_hiddens_mask.nonzero(), :]
            # else:
    ...
    def get_new_note(self,
                     ...
                     total_offset,
                     enforce_pitch=None,
                     enforce_duration=None):
        if isinstance(self.model, ChordPitchesModel):
            output_pitch, output_duration, new_hidden = self.model(
                last_note_net, hidden)
        else:
            output_pitch, output_duration, new_hidden, _ = self.model._forward(
                last_note_net, [], hidden)
        ...
        output_duration = self.enforce_triplet_and_small_fraction_completion(
            output_duration, total_offset)
        ...
        if self.score_model == 'reward':
            output_duration[:, self.score_inference.
                            reward_unsupported_durs] = -1e9

        if not (output_duration != -1e9).any(1).all():
            print('all durs -1e9')

        pitch_probs = F.softmax(output_pitch.squeeze() / self.temperature, -1)
        duration_probs = F.softmax(
            output_duration.squeeze() / self.temperature, -1)
        pitch_log_probs = F.log_softmax(
            output_pitch.squeeze() / (self.temperature), -1)
        ...
        if self.non_stochstic_search:
            ...
            new_pitch = torch.distributions.categorical.Categorical(
                probs=topp_p).sample().unsqueeze(-1)
            new_dur_net = torch.distributions.categorical.Categorical(
                probs=topp_d).sample().unsqueeze(-1)
        else:
        ...
        if self.score_model is not None and len(
                set(self.score_inference.reward_unsupported_durs).intersection(
                    new_dur_net)) > 0:
            print('PROBLEM!!')
        ...
        if enforce_duration is not None:
            new_dur_net = torch.tensor(enforce_duration,
                                       dtype=torch.long,
                                       device=self.device).expand(
                                           self.batch_size, 1)
        ...
        new_tie = torch.zeros([self.batch_size],
                              dtype=torch.long,
                              device=self.device)
        new_pitch_log_prob = torch.gather(pitch_log_probs, 1,
                                          new_pitch).detach().cpu().numpy()
        new_dur_log_prob = torch.gather(duration_log_probs, 1,
                                        new_dur_net).detach().cpu().numpy()
        new_note_log_prob = (new_pitch_log_prob +
                             new_dur_log_prob).squeeze() + 0.00001
    ...
    def handle_residual(self, measure_done, new_dur_m21, new_pitch, new_tie,
                        residuals_m21):
        residual_exists_mask = (residuals_m21[:, DURATION_IDX_IN_NOTE]
                                > 0) & (1 - measure_done.cpu().numpy())
        ...
        if at_least_one_residual_exists:
            new_pitch[residual_exists_indices] = self.object_to_tensor(
                np.asarray(residuals_m21[residual_exists_indices,
                                         PITCH_IDX_IN_NOTE]), int,
                new_pitch.dtype).unsqueeze(1)
            new_dur_m21[residual_exists_indices] = residuals_m21[
                residual_exists_indices, DURATION_IDX_IN_NOTE]
            ...
            residuals_larger_than_4_mask = self.object_to_tensor(
                ...
                float, torch.float) > 4
            continue_tensor = torch.ones_like(ties_with_residual).fill_(
                tie_2_value['continue'])
            end_tensor = torch.ones_like(ties_with_residual).fill_(
                tie_2_value['stop'])
            new_tie[residual_exists_indices] = torch.where(
                residuals_larger_than_4_mask, continue_tensor, end_tensor)

        measure_not_done_idxs = (1 - measure_done).nonzero()
        residuals_m21[measure_not_done_idxs.cpu().numpy(),
                      DURATION_IDX_IN_NOTE] = 0
        return residual_exists_mask

    def enforce_triplet_and_small_fraction_completion(self, output_duration,
                                                      total_offset):
        # TRIPLETS
        offset_is_triplet = np.asarray([(Fraction(t).denominator / 3) % 1 == 0
        ...
        no_whole_quarter_left_in_measure = np.asarray(
            [t % 4 > 3 for t in total_offset])
        cancel_triplets = (no_whole_quarter_left_in_measure *
                           (1 - offset_is_triplet)).nonzero()[0]
    ...
    def reset_history(self):
        if isinstance(self.model, ChordPitchesModel):
            self.initial_hidden = self.model.init_hidden(
